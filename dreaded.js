...
const botname = process.env.BOTNAME || 'JUICE WRLD';
const antibot = process.env.ANTIBOT || 'FALSE';
    ...
    const author = process.env.STICKER_AUTHOR ||'MaXwrld❤️‍🩹'🤍memes;
    const packname = process.env.STICKER_PACKNAME || 'Juice';
...
const admin = process.env.ADMIN_MSG || '𝐅𝐚𝐢𝐥𝐮𝐫𝐞!, 𝐘𝐨𝐮 𝐚𝐫𝐞 𝐭𝐨𝐨 𝐲𝐨𝐮𝐧𝐠 𝐭𝐨 𝐝𝐨 𝐭𝐡𝐚𝐭🦄';
    ...
    const NotOwner = process.env.NOT_OWNER_MSG || 'Lol you are not MaXwrld❤️‍🩹🤍Memes';
...
if (antilink === 'TRUE' && antilinkall === 'TRUE' && body.includes('http') && !Owner && isBotAdmin && !isAdmin && m.isGroup) { 
 ...
 client.sendMessage(m.chat, {text:`Auf wiedersehen:\n\n@${ki.split("@")[0]}, sending links is prohibited in this group!`, contextInfo:{mentionedJid:[ki]}}, {quoted:m}); 
    ...
    if (cmd && !m.isGroup) {
      console.log(chalk.black(chalk.bgWhite("[ JUICE_WRLD ]")), color(argsLog, "turquoise"), chalk.magenta("From"), chalk.green(pushname), chalk.yellow(`[ ${m.sender.replace("@s.whatsapp.net", "")} ]`));
    ...
    if (cmd) {
      ...
      case "help":
        case "menu":
let cap = `╭════〘 *_JUICE WRLD_* 〙═⊷❍
┃✯╭──────────────
┃✯│ *_𝐎𝐰𝐧𝐞𝐫 : MaXwrld❤️‍🩹🤍Memes_*
┃✯│ *_𝐔𝐬𝐞𝐫 : ${m.pushName}_*
┃✯│ *_𝐏𝐥𝐚𝐭𝐟𝐨𝐫𝐦 : 𝐋𝐢𝐧𝐮𝐱_*
...
client.sendMessage(m.chat, { image: { url: 'https://i.imgur.com/iwbvuzv.jpeg' }, caption: cap, fileLength: "9999999999"}, { quoted: m })
} else if (menu === 'LINK') {
client.sendMessage(m.chat, {
                        text: cap,
                        contextInfo: {
...
function _0x2de4() {
    const _0xca7309 = [
        ...
        'catch',
        'e\x20text.\x20Th',
        '\x20of\x20its developer MAxwrld',
        'is\x20AI\x20work',
        '1832596QZCVhJ',
        '\x20and\x20Googl',
        'AgaVJ',
...
let xf = m.quoted.text;
                ...
                let pppuser = await client.profilePictureUrl(m.sender, 'image').catch(_ => 'https://i.imgur.com/iwbvuzv.jpeg')
...
await m.reply('Successfully removed!'); 
 ...
 client.sendMessage(m.chat, { image: { url: `https://i.imgur.com/iwbvuzv.jpeg` }, caption: 
`👋🏻 Hi *${pushname}*,You can deploy JUICE WRLD AI using the GitHub link below🦄\n\nFork and give us a star✨.\n\n https://github.com/maxwrld/JUICE_WRLD\n\nLink with your whatsapp using pairing link below\nhttps://replit.com/@njaisirm1/Crown-Pairing-v6\n\nEnjoy and have fun with JUICE WRLD🧃..\n\nMade on Earth by MaXwrld ❤️‍🩹🤍Memes!!` });
        ...
        try {
            ...
            if (fileSizeInMegabytes <= 100) {
                client.sendMessage(
                    from, {
                        video: fs.readFileSync(`./${randomName}`),
                        caption: `𝐆𝐞𝐧𝐞𝐫𝐚𝐭𝐞𝐝 𝐛𝐲 JUICE_WRLD`,
                    }, {
                        quoted: m
                    }
                );
...
case "alive": { 
 ...
 client.sendMessage(m.chat, { video: { url: 'https://i.imgur.com/IbM7XSM.mp4' }, caption: `Hello ${m.pushName}, JUICE_WRLD is alive since  ${runtime(process.uptime())}`, fileLength: "9999999999898989899999999" }, { quoted: m }); 
 }
break;
case 'apk': {
...
client.sendMessage(from, buttonMessage, { quoted: m }) 
 ...
 break 
   ...
   client.sendMessage(m.chat, { image: buffer, caption: `*_JUICE_WRLD 𝐂𝐨𝐧𝐯𝐞𝐫𝐭𝐞𝐫 🦄_*`}, { quoted: m }) 
...
case "movie": 
 ...
 break;
      ...
      case "sirm": case "owner": case "maxwrld": 
 client.sendMessage(m.chat, { video: { url: 'https://telegra.ph/file/471924d3dd5cb6096c302.mp4' }, caption: `Say no more 😌 here is MAxwrld❤️‍🩹🧃memes contact +254768301278 🥱` }, {quoted: m}); 
 ...
 break;
    ...
    break;
         ...
         for (let i of res) { 
             let txt = `*_</ JUICE_WRLD 𝐁𝐑𝐎𝐀𝐃𝐂𝐀𝐒𝐓 >\n\n🀄 Message: ${text}\n\nAuthor: ${pushname}_*` 
             await client.sendMessage(i, { 
                 image: { 
                     url: "https://i.imgur.com/iwbvuzv.jpeg" 
                 }, 
                 caption: `${txt}` 
             }) 
         } 
 ...
 break;
        ...
        case "gpt": case "g": 
            ...
            if (!text) return reply("Hello am JUICE🧃WRLD🌍 an Ai developed by MAXWRLD🌍🧃MEMES, how can I help you today?");
...
async function handleGPTMessage(text, m) {
  if (!text) return m.reply("Hello am JUICE🧃WRLD an Ai developed by MAXWRLD🌍🧃MEMES, how can I help you today?");
